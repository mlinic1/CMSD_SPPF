WITH	STU_N	AS
		(SELECT	COUNT(student_id)			as student_sped_n
				,SCHOOL_CODE
				,SCHOOL_YEAR
		...
		READING AS
		(
		SELECT	count(FAY.student_id)		AS		prof_read_sped_n
		...
		MATH AS
		(
		SELECT	count(FAY.student_id)		AS		prof_math_sped_n
				,FAY.SCHOOL_CODE			AS		school_code
				,FAY.SCHOOL_YEAR			AS		school_year
		FROM SPPF..FAY_SUBGROUPS_V		AS	FAY
		LEFT JOIN SPPF..OAA_OGT_TESTS	AS	OAA
		...
		SCI AS
		(
		SELECT	count(FAY.student_id)		AS		prof_sci_sped_n
				,FAY.SCHOOL_CODE			AS		school_code
				,FAY.SCHOOL_YEAR			AS		school_year
		FROM SPPF..FAY_SUBGROUPS_V		AS	FAY
		LEFT JOIN SPPF..OAA_OGT_TESTS	AS	OAA
...
SELECT	stu_n.school_code
		,stu_n.school_year
		,isnull(stu_n.student_sped_n, 0)	AS	student_sped_n
		,isnull(prof_read_sped_n, 0)		AS	prof_read_sped_n
		,isnull(prof_math_sped_n, 0)		AS	prof_math_sped_n
		,isnull(prof_sci_sped_n, 0)			AS	prof_sci_sped_n
		,cast(isnull(prof_read_sped_n, 0) as float) / cast(isnull(student_sped_n, 0) as float)*100		
											AS	prof_read_sped_perc
		,cast(isnull(prof_math_sped_n, 0) as float) / cast(isnull(student_sped_n, 0) as float)*100		
											AS	prof_math_sped_perc
		,cast(isnull(prof_sci_sped_n, 0) as float) / cast(isnull(student_sped_n, 0) as float)*100			
											AS	prof_sci_sped_perc

/*
	   Name:		k8_oaa_sped_prof_v
	   Author:	Damico, Nicholas J
	   Purpose:	Query used to identify the percentage of K8 SPED students proficient in different subjects according 
	   to the OAA
	   Change Log:
	   Date		    Who				   What
	   *************   **********************  ****************
