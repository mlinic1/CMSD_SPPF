...
SELECT	school_code
		,school_year
		,COUNT(student_id)			AS	student_sped_n
		,SUM(prof_read)				AS	prof_read_sped_n
		,SUM(prof_math)				AS	prof_math_sped_n
		,SUM(prof_sci)				AS	prof_sci_sped_n
		,SUM(prof_soc)				AS	prof_soc_sped_n
		,SUM(prof_wri)				AS	prof_wri_sped_n
		...
		,CAST(sum(prof_soc) as float) / CAST(count(student_id) as float) *100
									AS	prof_soc_sped_perc
		,CAST(sum(prof_wri) as float) / CAST(count(student_id) as float) *100
									AS	prof_wri_sped_perc
/*
	   Name:		hs_ogt_sped_prof_v
	   Author:	Damico, Nicholas J
	   Purpose:	Query used to identify the percentage of 10th grade SPED students proficient in different subjects 
